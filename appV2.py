...
def load_equipment_db_from_sql():
    conn_str = st.secrets["mssql"]["connection_string"]
    engine = sqlalchemy.create_engine(conn_str)
    query = "SELECT SerialNumber, Model, EquipmentType, Specifications1 FROM EquipmentDB"
...
def process_single_sheet(input_df, ami_df):
    ...
    for _, row in ami_df.iterrows():
        ...
        serial_to_model[serial] = row['Model'] if pd.notna(row['Model']) else "MODEL MISSING"
        serial_to_type[serial] = row['EquipmentType'] if pd.notna(row['EquipmentType']) else "TYPE MISSING"
        serial_to_spec1[serial] = row['Specifications1'] if pd.notna(row['Specifications1']) else "N/A"
    ...
    for serial, model in serial_to_model.items():
        ...
        if model not in model_to_spec1:
            model_to_spec1[model] = serial_to_spec1[serial]
    ...
    grouped_models = sorted(
        [(model_to_type.get(model, "TYPE MISSING"), model, model_to_spec1.get(model, "N/A")) 
         for model in model_spares.keys()],
        key=lambda x: (x[0], x[1])
    )

    for equip_type, model, spec1 in grouped_models:
        output_rows.append([equip_type, model, spec1, '', '', '', '', ''])
    ...
    return pd.DataFrame(output_rows, columns=[
        'Equipment Type', 'Model', 'Specifications1', 'Total qty', 'Spare qty',
        'Item no.', 'Description', 'Unit Price ($)'
    ])

def process_excel(uploaded_file):
    input_excel = pd.ExcelFile(uploaded_file)
    ami_df = load_equipment_db_from_sql()
...
# ===== Streamlit Interface =====
st.title("🔧 Excel Re-organizer with SQL Sync")
